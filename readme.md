...
---
## 个人经验
ai coding目前来看还是你对他敷衍,他就对你敷衍.越是希望简单几句话就让ai自己琢磨透自己的意思,越是容易生成不好的结果,最后花很多时间修改后仍不理想,不如直接推到重做  
对于有一定复杂度的需求,不如先用一个md文档将需求描述,预期行为(用于结果验证),所需的上下文(接口字段描述 etc,也可以让ai根据需求来向自己获取上下文) 列出来.也不是一定要弄tdd,但至少自己要明确理解需求和预期,让ai也能够验证.之后可以让ai根据md文档生成计划,自己再验证一遍计划;这样往往一次生成就能得到效果不错的代码,有问题后续修改也很方便  
虽然一开始麻烦了很多,但是比起屡次生成都得不到满意的代码浪费时间,这样还是效率更高
...
- 等同于 **Agent.md** ,分项目和全局两种,可以自动生成.但是自动生成目前看来效果比较一般,有大量对于ai coding来说无效的信息
