...
对于有一定复杂度的需求,不如先用一个md文档将需求描述,预期行为(用于结果验证),所需的上下文(接口字段描述 etc) 列出来.也不是一定要弄tdd,但至少自己要明确理解需求和预期,让ai也能够验证.之后可以让ai根据md文档生成计划,自己再验证一遍计划;这样往往一次生成就能得到效果不错的代码,有问题后续修改也很方便  
